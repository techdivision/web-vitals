...
export default class ColumnView extends PureComponent {
    ...
    render() {
        ...
        return (
            <div>
                <div class={style.bar}>
                    <span class={[style.bar__legend, style.bar__left].join(' ')} style={{ width: low + '%' }}>
                        {thresholdMin}
                        {unit}&nbsp;&rsaquo;
                    </span>
                    <span
                        class={[style.bar__legend, style.bar__middle].join(' ')}
                        style={{ width: medium + '%' }}
                    ></span>
                    <span class={[style.bar__legend, style.bar__right].join(' ')} style={{ width: high + '%' }}>
                        &lsaquo;&nbsp;{thresholdMax}
                        {unit}
                    </span>
                </div>
                <div class={style.bar}>
                    <span class={[style.bar__section, style.bar__low].join(' ')} style={{ width: low + '%' }}>
                        {low}%
                    </span>
                    <span class={[style.bar__section, style.bar__medium].join(' ')} style={{ width: medium + '%' }}>
                        {medium}%
                    </span>
                    <span class={[style.bar__section, style.bar__high].join(' ')} style={{ width: high + '%' }}>
                        {high}%
                    </span>
                </div>
                <div class={style.infopanel}>
                    <div class={style.infopanel__date}>
                        {fromDate} - {toDate}
                    </div>
